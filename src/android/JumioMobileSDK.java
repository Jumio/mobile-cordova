...
public class JumioMobileSDK extends CordovaPlugin {
    ...
    private static final String ACTION_NV_SS_INIT = "initSingleSessionNetverify";
    ...
    public boolean execute(String action, JSONArray args, CallbackContext callbackContext) throws JSONException {
        ...
        if (action.equals(ACTION_BAM_INIT)) {
            ...
            result.setKeepCallback(false);
            return true;
        } else if (action.equals(ACTION_NV_SS_INIT)) {
            initSingleSessionNetverify(args);
            result = new PluginResult(Status.NO_RESULT);
            result.setKeepCallback(false);
            return true;
    ...
    private void initNetverify(JSONArray data) {
        ...
        try {
            ...
            if (!data.isNull(3)) {
                JSONObject options = data.getJSONObject(3);
                configureNetverify(netverifySDK, options);
            }
        } catch (JSONException e) {
            showErrorMessage("Invalid parameters: " + e.getLocalizedMessage());
        } catch (PlatformNotSupportedException e) {
    ...
    private void initSingleSessionNetverify(JSONArray data) {
        if (!NetverifySDK.isSupportedPlatform(cordova.getActivity())) {
            showErrorMessage("This platform is not supported.");
            return;
        }
        ...
        try {
            ...
            String authorizationToken = data.getString(0);

            JumioDataCenter dataCenter = null;
            try {
                dataCenter = JumioDataCenter.valueOf(data.getString(1).toUpperCase());
            } catch (Exception e) {
                throw new JSONException("Datacenter not valid: " + dataCenter);
            }

            netverifySDK = NetverifySDK.create(cordova.getActivity(), authorizationToken, dataCenter);

            // Configuration options
            if (!data.isNull(2)) {
                JSONObject options = data.getJSONObject(2);
                configureNetverify(netverifySDK, options);
            }
        } catch (JSONException e) {
            showErrorMessage("Invalid parameters: " + e.getLocalizedMessage());
    ...
    private void configureNetverify(NetverifySDK netverifySDK, JSONObject data) {
        try {
            ...
            while (keys.hasNext()) {
                ...
                if (key.equalsIgnoreCase("enableVerification")) {
                    netverifySDK.setEnableVerification(data.getBoolean(key));
                } else if (key.equalsIgnoreCase("callbackUrl")) {
                    netverifySDK.setCallbackUrl(data.getString(key));
                } else if (key.equalsIgnoreCase("enableIdentityVerification")) {
                    netverifySDK.setEnableIdentityVerification(data.getBoolean(key));
                } else if (key.equalsIgnoreCase("preselectedCountry")) {
                    netverifySDK.setPreselectedCountry(data.getString(key));
                } else if (key.equalsIgnoreCase("customerInternalReference")) {
                    netverifySDK.setCustomerInternalReference(data.getString(key));
                } else if (key.equalsIgnoreCase("reportingCriteria")) {
                    netverifySDK.setReportingCriteria(data.getString(key));
                } else if (key.equalsIgnoreCase("userReference")) {
                    netverifySDK.setUserReference(data.getString(key));
                } else if (key.equalsIgnoreCase("enableWatchlistScreening")) {
                    NVWatchlistScreening watchlistScreeningState;
                    switch (data.getString(key).toLowerCase()) {
                        case "enabled":
                            watchlistScreeningState = NVWatchlistScreening.ENABLED;
                            break;
                        case "disabled":
                            watchlistScreeningState = NVWatchlistScreening.DISABLED;
                            break;
                        default:
                            watchlistScreeningState = NVWatchlistScreening.DEFAULT;
                            break;
                    }
                    netverifySDK.setWatchlistScreening(watchlistScreeningState);
                } else if (key.equalsIgnoreCase("watchlistSearchProfile")) {
                    netverifySDK.setWatchlistSearchProfile(data.getString(key));
                } else if (key.equalsIgnoreCase("sendDebugInfoToJumio")) {
                    netverifySDK.sendDebugInfoToJumio(data.getBoolean(key));
                } else if (key.equalsIgnoreCase("dataExtractionOnMobileOnly")) {
                    netverifySDK.setDataExtractionOnMobileOnly(data.getBoolean(key));
                } else if (key.equalsIgnoreCase("cameraPosition")) {
                    JumioCameraPosition cameraPosition = (data.getString(key).toLowerCase().equals("front")) ? JumioCameraPosition.FRONT : JumioCameraPosition.BACK;
                    netverifySDK.setCameraPosition(cameraPosition);
                } else if (key.equalsIgnoreCase("preselectedDocumentVariant")) {
                    NVDocumentVariant variant = (data.getString(key).toLowerCase().equals("paper")) ? NVDocumentVariant.PAPER : NVDocumentVariant.PLASTIC;
                    netverifySDK.setPreselectedDocumentVariant(variant);
                } else if (key.equalsIgnoreCase("documentTypes")) {
                    JSONArray jsonTypes = data.getJSONArray(key);
                    ArrayList<String> types = new ArrayList<String>();
                    if (jsonTypes != null) {
                        int len = jsonTypes.length();
                        for (int i = 0; i < len; i++) {
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent intent) {
        ...
        if (requestCode == BamSDK.REQUEST_CODE) {
            ...
            if (resultCode == Activity.RESULT_OK) {
                ...
                String accountId = intent.getStringExtra(NetverifySDK.EXTRA_ACCOUNT_ID);
                Boolean authenticationResult = intent.getExtras().getBoolean(NetverifySDK.EXTRA_AUTHENTICATION_RESULT);
